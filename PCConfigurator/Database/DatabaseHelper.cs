...
namespace PCConfigurator
{
    public class DatabaseHelper
    {
        ...
        {
            List<Component> processors = new List<Component>();
            try
            {
                using (var connection = new SQLiteConnection("Data Source=Database\\configurator.db;Version=3;"))
                {
                    connection.Open();
                    string query = "SELECT Name, Price FROM Processors";
                    using (var cmd = new SQLiteCommand(query, connection))
                    using (var reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
            ...
            {
                MessageBox.Show("Ошибка при загрузке процессоров: " + ex.Message);
            }
            return processors;
        }
